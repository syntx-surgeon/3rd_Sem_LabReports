...
// Function to compute twoâ€™s complement
void twosComplement(int num[], int comp[])
{
    int carry = 1;
    for (int i = SIZE - 1; i >= 0; i--)
    {
        comp[i] = num[i] ^ 1; // Flip bits (1â€™s complement)
        // Add 1 to get 2â€™s complement
        if (carry)
        {
            if (comp[i] == 1)
                comp[i] = 0;
...
void restoringDivision()
{
    ...
    for (int i = 0; i < SIZE; i++)
    {
        ...
        addBinary(rem, divisorComp); // Subtract divisor (add twoâ€™s complement)
        cout << "Subtract B: ";
        for (int j = 0; j < SIZE; j++)
            cout << rem[j];
        cout << endl;
...
int main()
{
    ...
    twosComplement(divisorBin, divisorComp); // Get 2â€™s complement of divisor
